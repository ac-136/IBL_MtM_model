...
from utils.ibl_data_utils import (
    ...
    prepare_data, 
    select_brain_regions, list_brain_regions, 
    bin_spiking_data, create_intervals
)
...
idx = 150 # 03d9a098-07bf-4765-88b7-85f8d8f620cc from test
pid = bwm_df.pid[idx]
eid, probe = one.pid2eid(pid)
print(f"Session {idx} has PID {pid} and EID {eid}")

# # Load spike sorting data
# spikes, clusters, _ = load_spiking_data(one, pid)

# print("\nSpikes Keys:", spikes.keys())
...
params = {
    ...
    'n_trials': len(trials) # use for bin_spiking_data
}

# neural_dict, behave_dict, metadata = prepare_data(one, eid, bwm_df, params)
neural_dict, _, _, _ = prepare_data(one, eid, bwm_df, params)

# print("\nneural_dict keys:", neural_dict.keys())
...
print("\nIntervals: ")
print("end time: ", neural_dict['spike_times'].max())
print(intervals)
print(intervals.shape)

binned_spikes, clusters_used_in_bins = bin_spiking_data(reg_clu_ids=None, intervals=intervals, neural_df=neural_dict, **params)
...
dataset = create_dataset(binned_spikes, just_spikes=True, bwm_df=bwm_df, eid=eid, params=None, binned_behaviors=None, meta_data=None)
...
# print(f"\nShape of downloaded binned spikes: {downloaded_binned_spikes.shape}. Shape of original binned spikes: {binned_spikes.shape}")