...
from utils.ibl_data_utils import (
    ...
    bin_spiking_data, create_intervals, bin_behaviors
)
...
neural_dict, behave_dict, metadata, trials_data = prepare_data(one, eid, bwm_df, params, n_workers=8)
...
plt.ylabel("unit")
plt.colorbar()
plt.savefig("spike_count_trial_{}.png".format(interval_idx))
...
binned_behaviors, behavior_masks = bin_behaviors(one, eid, trials, allow_nans=True, **params)
...
plt.xlabel("time bin")
plt.ylabel(behave)
plt.savefig(f"{behave}_trial_{interval_idx}.png")
