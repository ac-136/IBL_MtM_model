...
def parse_log(path: Path) -> dict:
    ...
    if "train_loss" not in best_metrics or "eval_loss" not in best_metrics or "r2" not in best_metrics:
        raise ValueError(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "train-sessions-folds log files."
        )
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=Path("script"),
        help="Directory containing train-sessions-folds-*.out files.",
    )
    parser.add_argument(
        "--pattern",
        default="train-sessions-folds-*.out",
